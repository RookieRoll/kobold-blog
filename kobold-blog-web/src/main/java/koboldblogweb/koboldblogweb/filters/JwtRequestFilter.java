...
//		}
        String authCode = authManager.getAuthCode(controllerName);
        if (!StringUtils.isEmpty(authCode)) {
            Optional<? extends GrantedAuthority> authority = userDetails.getAuthorities().stream().filter(m -> ((GrantedAuthority) m).getAuthority().equals(authCode)).findFirst();
            if (!authority.isPresent()) {
                throw new BadCredentialsException("您没有访问的权限");
            }
        }
    }
}
